#!/usr/bin/env python3
"""
Email test with SSL certificate fix
"""
...
def test_email():
    """Test email sending functionality with SSL fix"""
...
SRM Traffic System
    ...
    try:
        ...
        # Try alternative method with STARTTLS
        try:
            print("Trying alternative STARTTLS method...")
            with smtplib.SMTP(smtp_server, 587) as server:
                server.starttls(context=context)
                server.login(email_address, email_password)
                server.send_message(message)
            
            print("SUCCESS: Test email sent via STARTTLS!")
            return True
            
        except Exception as e2:
            print(f"ERROR: STARTTLS also failed: {str(e2)}")
            return False
